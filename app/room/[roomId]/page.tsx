...
export default function RoomPage() {
  ...
  const [receivedSnitchMessage, setReceivedSnitchMessage] = useState<{
    message: string;
    snitchName: string;
  } | null>(null);
  const [voteResults, setVoteResults] = useState<Record<
    string,
    string[]
  > | null>(null);
  ...
  const [killLocked, setKillLocked] = useState<{
    id: string;
    name: string;
  } | null>(null);
  ...
  useEffect(() => {
    ...
    const playersUpdateHandler = (data: any) => {
      const playersObj = data.players.reduce(
        (acc: Record<string, Player>, p: Player) => {
          acc[p.id] = p;
          return acc;
        },
        {}
    ...
    const phaseChangeHandler = (data: { phase: Phase; round: number }) => {
      setRoom((prev) => ({
        ...
        round: data.round,
        chat:
          data.phase === "night" || data.phase === "discussion"
            ? []
            : prev?.chat || [],
      }));
    ...
    const handleWerewolfKilled = (data: {
      targetId: string;
      targetName: string;
      usesLeft: number;
    }) => {
  ...
  const canStartGame =
    isHost && Object.keys(room.players || {}).length >= 4 && !room.gameStarted;
      ...
      <div className="max-w-7xl mx-auto space-y-4">
        ...
        {showingVoteResults && voteResults && room.phase === "results" && (
          <div className="bg-orange-900/90 border-2 border-orange-500 rounded-lg p-6 shadow-2xl">
            <h2 className="text-2xl font-bold text-orange-300 mb-4 text-center">
              📊 Vote Results
            </h2>
            <div className="space-y-3">
              ...
              {Object.entries(voteResults).map(([votedId, voters]) => {
                const votedPlayer = Object.values(room.players || {}).find(
                  (p) => p.id === votedId
                );
                return (
                  <div
                    key={votedId}
                    className="bg-slate-800/70 rounded-lg p-3 border border-orange-600/50"
                  >
                    <p className="font-bold text-orange-200 mb-2">
                      {votedPlayer?.name} received {voters.length} vote
                      {voters.length !== 1 ? "s" : ""}:
                    </p>
                    <p className="text-gray-300 text-sm">{voters.join(", ")}</p>
                  </div>
                );
              })}
            </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-4 gap-4">
          ...
          <div className="lg:col-span-3 space-y-4">
            ...
            {!room.gameStarted && (
              <div className="bg-slate-800/50 p-6 rounded-lg border border-slate-700 text-center space-y-4">
                <p className="text-gray-300 mb-4">
                  {Object.keys(room.players || {}).length}/4-8 players
                </p>
            ...
            {room.gameStarted &&
              (room.phase === "night" || room.phase === "healer") &&
              currentPlayer && (
                <NightPhase
