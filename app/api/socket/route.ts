...
function checkMidGameQuit(io: Server, roomId: string) {
  ...
  const totalWerewolves = Object.values(room.players).filter(p => p.role === "werewolf").length
  ...
  if (aliveWerewolves.length === 0 && totalWerewolves > 0 && allWerewolvesQuit) {
    room.phase = "ended"
    io.to(roomId).emit("game-end", { winner: "villagers", reason: "All werewolves quit" })
    return true
  }

  if (aliveWerewolves.length >= aliveNonWerewolves.length && aliveNonWerewolves.length <= 1) {
    room.phase = "ended"
    io.to(roomId).emit("game-end", { winner: "werewolves", reason: "Other players quit" })
    return true
  }
...
function handleNightPhaseEnd(io: Server, roomId: string) {
  ...
  if (!room.werewolfKilledId) {
    const alivePlayers = Object.values(room.players).filter(p => p.alive && p.role !== "werewolf")
  ...
  const healerAlive = Object.values(room.players).some(p => p.role === "healer" && p.alive)
...
export async function GET(req: Request) {
  console.log("[v0] Socket.io route handler called")
  ...
  io.on("connection", (socket) => {
    ...
    socket.on("join-room", (roomId: string, playerName: string, callback) => {
      ...
      if (existingPlayer) {
        ...
        callback({ playerId: existingPlayer.id, success: true, reconnected: true })
        return
      }
    ...
    socket.on("start-game", (callback) => {
      ...
      players.forEach((player, index) => {
        ...
        if (player.role === "healer") {
          room.healerUsesLeft[player.id] = 2 // Healer starts with 2 lives
          console.log(`[HEALER INIT] ${player.name} initialized with 2 lives`)
        }
      })
    ...
    socket.on("werewolf-kill", (targetId: string) => {
      ...
      if (!senderId || room.players[senderId].role !== "werewolf" || !room.players[senderId].alive) {
